...
// DailyCrashesByOs will query the Crittercism API and get the daily crashes by os
// It will then emit a Flow object to the flowChan for sending up to Telemetry
func DailyCrashesByOs(flowChan chan gotelemetry.Flow, config map[string]string) {

	const interval = 3600
	const label = "Daily Crashes By OS"
	const tag = "daily_crashes_by_os"
